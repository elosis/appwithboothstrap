...
import CarouselBooksModal from "../modals/CarouselBooksModal";
...
const CarouselBooks = () => {
  ...
  const { id } = useParams();
  const [singleBook, setSingleBook] = useState<CarouselProps | null>();

  useEffect(() => {
    const getData = async () => {
      await axios
        .get<CarouselProps[]>(
          `https://sfvmzovrujwtnthorsww.supabase.co/rest/v1/MainPage?id=eq.${id}&select=*`,
          {
            headers: {
              apikey:
    ...
    <div>
      ...
      {singleBook ? (
        <div className="container">
          <div className="card mb-3 mx-auto mt-4 border-0">
            <div className="row g-0">
              <div className="col-md-4">
                <img
                  src={singleBook?.imageUrlSecond}
                  alt="..."
                  className="w-75"
                />
